"""
Based on code from the xDSL project.

https://github.com/xdslproject/xdsl/blob/806d0833832a4daf15c721cb17afd6aea962f119/xdsl/utils/scoped_dict.py
"""
...
class ScopedDict(Generic[_Key, _Value]):
    ...
    A ScopedDict may have a parent dict, which is used as a fallback when a
    value for a key is not found.
    If a ScopedDict and its parent have values for the same key, the child
    value will be returned.
    This structure is useful for contexts where keys and values have a known
    scope, such as during IR construction from an Abstract Syntax Tree.
